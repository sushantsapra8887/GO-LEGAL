      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="space-y-6" ref={cardRef}>
          {points.map((point, idx) => (
            <motion.div
              key={idx}
              className="bg-white p-4 rounded-xl shadow hover:shadow-lg transition border flex items-start space-x-4"
              whileHover={{ scale: 1.02 }}
            >
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold ${bgColors[idx % bgColors.length]}`}
              >
                <span className={`${iconColors[idx % iconColors.length]}`}>{point.icon}</span>
              </div>
              <div>
        ...
        {/* Right: Image exactly matching card height */}
        <div className="flex justify-center items-start">
          <div
            className="rounded-3xl overflow-hidden shadow-lg border-4 border-blue-200 max-w-md w-full"
